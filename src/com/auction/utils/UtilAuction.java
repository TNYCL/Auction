...
public class UtilAuction {
	...
	public static void createNewAuction(String owner, ItemStack itemdata, int price, int amount) {
		FindIterable<Document> document = Auction.getMongo().getAuctionDatabase().find()
				.sort(new Document("auctionid", -1)).limit(1);
		
		int last_id = 0;
		if(Auction.getSpring().mongoTemplate().getCollection("auction").count() == 0) {
			last_id = 1;
		} else {
	...
	public static boolean isAuctionIdHave(int auctionid) {
		if(Auction.getSpring().getAuctionData("auctionid", auctionid) == null) {
			return false;
		}
		return true;
	}
